...
fn check_sigaction(sig_action: &SigAction) -> Result<()> {
    ...
    cfg_if::cfg_if! {
        if #[cfg(target_arch = "x86_64")] {
            ...
            // On RISC-V and AArch64, if `SA_RESTORER` is not specified, the kernel
            // provides a signal-return trampoline (`__vdso_rt_sigreturn` on RISC-V,
            // a per-process executable page on AArch64).
            Ok(())
        } else {
