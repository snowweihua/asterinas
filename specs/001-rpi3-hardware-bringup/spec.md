...
- U-Boot is pre-configured with TFTP boot commands; the boot script (`boot.scr`) loads `asterina.img` and the uncompressed `initramfs.cpio` via TFTP and passes them via DTB `linux,initrd-start/end`
- Serial console is connected via PL011 UART at 115200 baud, 8N1, to a host machine
- A TFTP server is running on the dev machine (Windows TFTP root `D:/pi_sd/`, WSL2 mount `/mnt/d/pi_sd/`) serving `asterina.img` and the uncompressed `initramfs.cpio`; `/srv/tftp` is not used
