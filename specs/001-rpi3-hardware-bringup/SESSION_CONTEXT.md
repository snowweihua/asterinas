...
- QEMU currently reaches the shell prompt intermittently, but `ls` echoes and hangs without a listing. A temporary trace showed the shell reaches AArch64 `clone` with flags `0x1200011`, then stalls in the child-process fork/COW address-space path before `execve` or `getdents64`; no fix is committed yet. Some QEMU boots also stall during initial init scheduling before the prompt, so the scheduler/TLB boundary remains under investigation.
