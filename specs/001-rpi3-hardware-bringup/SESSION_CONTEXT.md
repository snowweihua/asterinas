...
- The QEMU virt image reaches the Asterinas banner and completes component initialization after the GIC/timer changes. The documented `test/build/initramfs.cpio` is 44,051,456 bytes, while the previous QEMU hardcoded range ended 9,728 bytes early and caused `CPIO buffer is too short`; the hardcoded end was corrected to `0x5aa0_2c00`. The QEMU GIC path also needed Group 1 forwarding (`GICC_CTLR=0b111`) and a Device-mapped linear GIC window: direct low-VA `GICC_IAR`/`GICC_EOIR` accesses faulted after the first user address space was installed. AArch64 lower-EL IRQ vectors now use a dedicated save/callback/restore/`eret` trampoline instead of the synchronous user-context path. This removed dynamic-userspace corruption after timer IRQs; a clean diagnostic run reached the `~ #` shell prompt without page faults or SIGSEGV. The exact serial capture still needs a final no-diagnostics run because the large colored banner can fill the QEMU PL011 output path.
