...
- The QEMU virt image reaches the Asterinas banner and completes component initialization after the GIC/timer changes. The documented `test/build/initramfs.cpio` is 44,051,456 bytes, while the previous QEMU hardcoded range ended 9,728 bytes early and caused `CPIO buffer is too short`; the hardcoded end was corrected to `0x5aa0_2c00`. The QEMU GIC path also needed Group 1 forwarding (`GICC_CTLR=0b111`) and a Device-mapped linear GIC window: direct low-VA `GICC_IAR`/`GICC_EOIR` accesses faulted after the first user address space was installed. Those faults are gone and timer IRQ acknowledgement is observed, but the current QEMU userspace console/scheduling validation remains open: the shell has not yet produced a reliable `/ #` prompt on serial.
