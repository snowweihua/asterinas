...
**B6 notes**: Root-caused the intermittent EL1 fault under concurrent fork (slab lists held stale bootstrap-physical pointers — the heap missed the frame allocator's pointer conversion; plus all slab lists shared ID 1, misrouting removals). Fixed by heap bootstrap conversion (global + per-CPU, at ostd init + AP entry) and unique list IDs; non-atomic `inc_count`/`Mutex` RPi3 paths also hardened to atomic ops. Verified: eater stress clean 4x QEMU + HW stress clean, smoke PASSED. Frame refcounts (`inc`/`dec`/`mark_unique`) unified to atomics too; HW stress clean on that image. QEMU-only spawn SEGV flake needs re-characterization on the fixed image under C102/C103. UPDATE: AP wait replaced with sevl/wfe/sev (`wait_for_ap_late_entry`, aarch64-only) — 2/2 QEMU boots all APs reach idle-halt, HW boots clean to prompt. F2 DECISIVE EVIDENCE: faulting insn is the `ldxr` in `ThreadOptions::build` (alloc + 2 successful stores precede it — translation proven valid, not a page-table bug); same victim VA 4x; system survives (shell works post-HALT); verdict = silicon exclusive-handling quirk, dumps in `stress-fault-hw*.log`; rate looks elevated on new image (3 faults / 2 boots) — keep monitoring across future boots.
