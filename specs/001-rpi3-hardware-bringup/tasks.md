...
**B6 notes**: Root-caused the intermittent EL1 fault under concurrent fork (slab lists held stale bootstrap-physical pointers — the heap missed the frame allocator's pointer conversion; plus all slab lists shared ID 1, misrouting removals). Fixed by heap bootstrap conversion (global + per-CPU, at ostd init + AP entry) and unique list IDs; non-atomic `inc_count`/`Mutex` RPi3 paths also hardened to atomic ops. Verified: eater stress clean 4x QEMU + HW stress clean, smoke PASSED. QEMU-only spawn SEGV flake needs re-characterization on the fixed image under C102/C103.
