...
- [ ] C103 Stress `fork`, `vfork`, `clone`, `wait4`, `execve`, signals, and TLS-sensitive dynamic programs. Partial: fork/exec/wait4 churn clean (eater stress both targets, F1 era); yield/sleep per-CPU (probes v2/v3); SIGTERM delivery+reap via shell. Open: vfork/clone specifics, TLS-sensitive dynamic programs.
