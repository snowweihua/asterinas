...
**B6 notes**: Root-caused the intermittent EL1 fault under concurrent fork (slab lists held stale bootstrap-physical pointers — the heap missed the frame allocator's pointer conversion; plus all slab lists shared ID 1, misrouting removals). Fixed by heap bootstrap conversion (global + per-CPU, at ostd init + AP entry) and unique list IDs; non-atomic `inc_count`/`Mutex` RPi3 paths also hardened to atomic ops. Verified: eater stress clean 4x QEMU + HW stress clean, smoke PASSED. Frame refcounts (`inc`/`dec`/`mark_unique`) unified to atomics too; HW stress clean on that image. QEMU-only spawn SEGV flake needs re-characterization on the fixed image under C102/C103. UPDATE: AP wait replaced with sevl/wfe/sev (`wait_for_ap_late_entry`, aarch64-only) — 2/2 QEMU boots all APs reach idle-halt, HW boots clean to prompt. **HW AP SCHEDULING PROVEN**: affinity probe as `/init` (fork 3 kids pinned to CPU1/2/3, all ALIVE+reaped; source `.github/agent_state/ap_probe.c`) — first direct evidence all APs execute user threads on hardware; `sched_setaffinity` works. F2 DECISIVE EVIDENCE: faulting insn is the `ldxr` in `ThreadOptions::build` (alloc + 2 successful stores precede it — translation proven valid, not a page-table bug); same victim VA 4x; system survives (shell works post-HALT); verdict = silicon exclusive-handling quirk, dumps in `stress-fault-hw*.log`; rate looks elevated on new image (3 faults / 2 boots) — keep monitoring across future boots. UPDATE2: 2nd fault site ID'd (`WaitTimeout::wait_until_or_timeout`, same fresh-alloc+stores+`ldxr`-abort pattern); working model = snoop/exclusive SLVERR on same-line neighbor collision; spawner-ID dump deployed (image 3917232 prints CPU=/TASK=), awaiting next fault.
...
- [x] C102 Run shell loops for `echo`, `true`, `ls`, `stat`, `lstat`, `cat /proc/interrupts`, file creation/removal, and symlink operations. Done on HW serial shell: for-loops + `true` x5, `ls`, `stat` (incl. symlink lstat), file create/read/remove, symlink create/read-via-link/remove, `nproc`=4, `uname`, `uptime` (load 0.00), taskset-pinned echo on AP1/2/3 + pinned `sleep`, SIGTERM delivery+reap. Full applet set reachable via `busybox <applet>`. Restrictions: `/proc/interrupts` absent and `ps` enumerates nothing (minimal procfs).
- [x] C103 Stress `fork`, `vfork`, `clone`, `wait4`, `execve`, signals, and TLS-sensitive dynamic programs. Done with documented boundaries: fork/exec/wait4 churn clean (eaters both targets); vfork + clone(SIGCHLD) + sigaction/kill pass on BOTH targets (probe4b, `.github/agent_state/ap_probe4b.c`); SIGTERM via shell. Boundaries (both targets): static pthread binaries die at NPTL TLS setup (`page fault ...ffe0` = TP-32, sustained handler loop that never kills the task — kernel robustness gap filed); `CLONE_SYSVSEM` explicitly unsupported; TLS-sensitive dynamic programs untested (no loader/libs on target).
- [x] C104 Repeat reboot tests after A3 is complete. Done: A3 gate complete (A301-A304); `busybox reboot -f` via serial shell → PSCI reset → fresh firmware boot from 00:00:01 → TFTP → AUTO-TEST-DONE + `/ #` prompt.
- [x] C105 Classify network/TFTP failures separately from kernel and userspace failures. Done: benign class = DHCP-probed `0A8E0F*.img` 'File not found' auto-falling-back to `asterina.img` (every boot, infra-normal); flake class = occasional initramfs TFTP retry auto-reset clean (C101 cycles 2/12); distinct from kernel faults (EL1-SYNC) and userspace failures.
...
- [x] C107 (moved from B603) Run SMP stress tests and single-core fallback validation. Done: probe v3 (4-way parallel pinned spins + 4x200k shared-atomic race; source `.github/agent_state/ap_probe3.c`) clean on BOTH targets — QEMU spins 50-133ms + atomic exactly 800000; HW spins 4x17ms identical (true parallel) + atomic exactly 800000. Single-core: QEMU 1-CPU DTB (`/tmp/smp1.dtb`, cpu@1..3 removed) boots to prompt+echo. HW single-core BLOCKED: pruned 1-CPU DTB on SD (33888 bytes, confirmed live by firmware size 0x87cf = normal − 799) halts reproducibly after TF-A BL31, before U-Boot (2 attempts) — BL31 PSCI topology init chokes on the pruned DTB. Board unusable until original DTB restored; QEMU single-core result stands. UPDATE: user restored the original SD DTB; recovery boot verified clean (firmware reads 34687-byte DTB, full boot to prompt, no fault).

### C2 — Performance and capacity

- [x] C201 Measure power-on-to-prompt and TFTP transfer times over a representative sample. Done (~20+ boots): power-on to `/ #` prompt ~50s; TFTP kernel (3.7MiB) ~2s + initramfs (4.5MiB) ~3s @ ~1.5-2MiB/s; U-Boot 5s autoboot countdown included.
- [x] C202 Track initramfs size and identify safe closure reductions after correctness is stable. Done: good image = 27 entries, 4,678,250 bytes; libc.so.6 2.25MB + busybox 1.42MB + libm 0.69MB + ld.so 0.23MB + libresolv 79KB dominate. Reduction candidates (need verification before removal): libresolv/libm if no dynamic user needs them; probe cpios run ~650-720KB uncompressed.
- [x] C203 Revisit compressed initramfs only after a reliable AArch64 decompression path is proven; do not use the known broken U-Boot inflate path. Done: kernel-side GZipDecoder proven on HW — gzip'd probe2 (299,451 bytes, was 660,480) booted, unpacked, full PROBE2 pass on all 4 CPUs. U-Boot inflate path untouched (decompression happens in kernel rootfs init).

### C3 — Architecture and shared-code cleanup

- [x] C301 Remove stale debug strings, empty conditionals, contradictory comments, and obsolete task references. Done (cleanup image 3917232, QEMU+HW boot clean): collapsed identical if/else around `boot_all_aps()`, removed empty RPi3 `if` arm around `io::init`, updated stale single-core `tsc_freq` comment, removed RISC-V FIXME in aarch64 `read_random`, delegated serial `is_rpi3` duplicate to `crate::arch::is_rpi3`. Deliberately kept: `[a2-smp]` INFO logs (level-gated diagnostics) and QEMU TLBI workarounds (still load-bearing until proven otherwise).
- [ ] C302 Consolidate RPi3 board detection, timer policy, IRQ setup, and single-core helpers behind clear board-specific abstractions. Partial: board dispatch hoisted to a single `is_rpi3_board` in `late_init_on_bsp`; serial `is_rpi3` delegates to the shared one. Assessment: full trait-style abstraction deferred per C305 — per-site dispatch is explicit and boot-verified; deeper refactoring buys style at regression risk on the verified boot path.
- [x] C303 Audit direct `spin::Once`, exclusive atomic, 16-byte aggregate-return, and hard-coded physical-address use in AArch64 paths. Done, no changes needed: `spin::Once` only in `rpi3_watchdog.rs` (one-shot init, benign); exclusives only in exception-guarded `memcpy_fallible.S` plus proven EL1 atomics; all hardcoded PAs match the corrected map (0x40000000 ARM-local / 0x3F000000 peripherals); 0x3F201000 in `ap_boot.S` is correct for both HW and QEMU-raspi3b.
- [x] C304 Keep architecture-specific unsafe code under `ostd/src/arch/aarch64/` and document safety assumptions. Done: containment verified (kernel `arch/aarch64` has zero `unsafe`; all 170 sites live under `ostd/src/arch/aarch64`, concentrated in bcm2836_irq/serial/smp_rpi3/mm/trap); `# Safety` module docs added to those five files. Verified: comment-only image 3917232 boots clean QEMU+HW.
