...
**B6 notes**: Root-caused the intermittent EL1 fault under concurrent fork (slab lists held stale bootstrap-physical pointers — the heap missed the frame allocator's pointer conversion; plus all slab lists shared ID 1, misrouting removals). Fixed by heap bootstrap conversion (global + per-CPU, at ostd init + AP entry) and unique list IDs; non-atomic `inc_count`/`Mutex` RPi3 paths also hardened to atomic ops. Verified: eater stress clean 4x QEMU + HW stress clean, smoke PASSED. Frame refcounts (`inc`/`dec`/`mark_unique`) unified to atomics too; HW stress clean on that image. QEMU-only spawn SEGV flake needs re-characterization on the fixed image under C102/C103. OPEN: rare post-prompt external abort on table walk in thread-spawn path (2-3 HW sightings, full dumps in `.github/agent_state/stress-fault-hw*.log`, never on QEMU; suspected RPi3 AXI-exclusive behavior, needs dedicated hunt).
