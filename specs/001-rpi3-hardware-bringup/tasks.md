...
- [ ] C302 Consolidate RPi3 board detection, timer policy, IRQ setup, and single-core helpers behind clear board-specific abstractions. Partial: board dispatch hoisted to a single `is_rpi3_board` in `late_init_on_bsp`; serial `is_rpi3` delegates to the shared one. Timer policy/IRQ setup/single-core helpers still per-site — full abstraction deferred.
