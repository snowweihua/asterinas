...
fn parse_memory_regions() -> MemoryRegionArray {
    ...
    if crate::arch::board::BoardType::cached() != 2 {
        regions
            ...
            .unwrap();
        // GIC + UART live below DRAM; NonVolatileSleep keeps them linearly
        // mapped (as physical) without handing them to the frame allocator.
        regions
            .push(MemoryRegion::new(
                0x0800_0000,
                0x0200_0000,
        ...
        // SoC peripherals + ARM-local registers live above DRAM; NonVolatileSleep
        // keeps them linearly mapped (as physical) without handing them to the
        // frame allocator. Without this, post-activation MMIO faults endlessly
        // because the fault handler itself prints through the same mapping.
        regions
            .push(MemoryRegion::new(
                0x3F00_0000,
                0x0110_0000,
...
fn discover_dtb_paddr(device_tree_paddr: usize) -> Option<usize> {
    ...
    // Only probe the QEMU-virt loader address when the firmware gave us
    // nothing at all; on RPi3 (ATAGS at 0x100) probing 0x47000000 faults
    // because it is unmapped physical address space.
    if device_tree_paddr == 0 && is_valid_dtb_paddr(QEMU_LOADER_DTB_PADDR, DTB_SCAN_END) {
...
pub unsafe extern "C" fn aarch64_boot(device_tree_paddr: usize, _reserved: usize) -> ! {
    use crate::boot::{start_kernel, EarlyBootInfo, EARLY_INFO};
