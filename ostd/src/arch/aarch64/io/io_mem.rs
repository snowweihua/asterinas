...
pub(crate) unsafe fn read_once<T: PodOnce>(ptr: *const T) -> T {
    // Volatile accesses to u8-u64-sized types compile to single ldr/str
    // instructions on AArch64, satisfying the guarantee above.
...
pub(crate) unsafe fn write_once<T: PodOnce>(ptr: *mut T, val: T) {
    // Same single-instruction note as in `read_once`.
